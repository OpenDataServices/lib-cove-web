            ...
            if form_name == "url_form":
                try:
                    ...
                    return render(
                        ...
                        context={
                            "sub_title": _(
                                "Sorry we got a ConnectionError whilst trying to download that file"
                            ),
                            "link": "index",
                ...
                data.original_file.save(
                    text_file_name, ContentFile(form["paste"].value())
                )
            return redirect(data.get_absolute_url())

    return render(
        request,
        settings.COVE_CONFIG.get("input_template", "input.html"),
        {"forms": forms},
    )
